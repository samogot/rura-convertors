...
function _htmltodocx_get_style($element, $state)
{
    ...
    $tag_style     = isset($style_sheet['elements'][$element->nodeName]) ? $style_sheet['elements'][$element->nodeName] : array();
    ...
    if (!empty($element->getAttribute('style'))) {
        $inline_styles = explode(';', rtrim(rtrim($element->getAttribute('style')), ';'));
 ...
 *
 * @param \PhpOffice\PhpWord\PhpWord $phpword_object
 *   PhpWord object
 * @param DOMNode                    $html_dom_array
...
function htmltodocx_insert_html(&$phpword_object, $html_dom_array, &$state = array())
{
    ...
    // Tables:
    if (in_array('td', $state['parents']) || in_array(
            'th',
            $state['parents']
        ) || (isset($state['table_allowed']) && !$state['table_allowed'])
    ) {
 ...
 * @param       \PhpOffice\PhpWord\Element\AbstractElement phpword_element
...
{
    switch ($html_dom_array->nodeType) {
        case XML_TEXT_NODE:
            $state['textrun']->addText(htmltodocx_clean_text($html_dom_array->nodeValue), $state['current_style']);
            break;
        case XML_ELEMENT_NODE:
            ...
            foreach ($html_dom_array->childNodes as $element) {
                if (!in_array($element->nodeType, [XML_ELEMENT_NODE, XML_TEXT_NODE])) {
                    continue;
                }
                ...
                switch ($tag) {
                    ...
                    case 'h6':
                    case 'center':
                        if ($state['structure_document'] && in_array($tag, ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']) &&
                            is_object($state['phpword_object'])
                        ) {
                            ...
                            $state['phpword_object']->addTitleStyle(
                                $state['structure_headings'][$tag],
                                $state['current_style']
                            );
                        ...
                        if ($tag == 'div' && $state['table_of_contents_id'] &&
                            $element->getAttribute('id') == $state['table_of_contents_id']
                        ) {
                        ...
                        if ($tag != 'div' || $state['treat_div_as_paragraph'] || !isset($state['textrun'])) {
                            if (empty($phpword_element)
                                || $phpword_element->getSettings()->getMarginTop() === 0
                            ) {
                                $phpword_element = $phpword_object->createSection();
                        ...
                        if (in_array($tag, $allowed_children)) {
                            array_unshift($state['parents'], $tag);
                            $phpword_element_returned = htmltodocx_insert_html_recursive(
                                $phpword_object,
                                $phpword_element,
                                $element,
                                $state
                            ...
                            if (!empty($phpword_element)
                                && ($phpword_element_returned->getSettings()->getOrientation(
                                    ) !== $phpword_element->getSettings()->getOrientation())
                            ) {
                                $phpword_element = $phpword_object->createSection(
                                    array('orientation' => $phpword_element->getSettings()->getOrientation())
                                );
                            } else {
                    ...
                    case 'table':
                        if (in_array('table', $allowed_children)) {
                            ...
                            if (!$state['table_allowed'] || in_array('td', $state['parents']) || in_array(
                                    'th',
                                    $state['parents']
                                )
                            ) {
                                ...
                                if (is_object($state['phpword_object']) && method_exists(
                                        $state['phpword_object'],
                                        'addTableStyle'
                                    )
                                ) {
                    ...
                    case 'td':
                    case 'th':
                        if (in_array($tag, $allowed_children) && $state['table_allowed']) {
                            unset($state['textrun']);
                            if (isset($state['current_style']['width'])) {
                                $cell_width = $state['current_style']['width'];
                            } elseif (!empty($element->getAttribute('width'))) {
                                $cell_width = $element->getAttribute('width') * 15;
                    ...
                    case 'a':
                        ...
                        if ($state['context'] == 'section') {
                            ...
                            if (strpos($element->getAttribute('href'), 'http://') === 0) {
                                $href = $element->getAttribute('href');
                            } else if (strpos($element->getAttribute('href'), 'https://') === 0) {
                                $href = $element->getAttribute('href');
                            } elseif (strpos($element->getAttribute('href'), '/') === 0) {
                                $href = $state['base_root'] . $element->getAttribute('href');
                            } else {
                                $href = $state['base_root'] . $state['base_path'] . $element->getAttribute('href');
                            }
                            // Replace any spaces in url with %20 - to prevent errors in the Word
                            // document:
                            $state['textrun']->addLink(
                                htmltodocx_url_encode_chars($href),
                                htmltodocx_clean_text($element->nodeValue),
                                $state['current_style']
                            );
                        } else {
                    ...
                    case 'ul':
                        $state['list_total_count'] = $element->childNodes->length;
                    ...
                    case 'text':
                        ...
                        if (!isset($state['textrun'])) {
                            $text = htmltodocx_clean_text(trim($element->nodeValue));
                        } else {
                            $text = htmltodocx_clean_text($element->nodeValue);
                        }
                        if (!empty($text)) {
                            if (!isset($state['textrun'])) {
                                if (empty($phpword_element)) {
                    ...
                    default:
                        ...
                        $state['textrun'] = $phpword_element->createTextRun();
                        $state['textrun']->addText(htmltodocx_clean_text($element->nodeValue), $state['current_style']);
                        break;
                }
